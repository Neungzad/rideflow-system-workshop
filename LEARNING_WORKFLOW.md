...
> How to use SPEC.md + Claude Code to learn AND build at the same time
...
The difference: you always know WHY before Claude Code generates the HOW.
...
### 2. Create CLAUDE.md at Repo Root

This tells Claude Code the full context of your project.  
**Critical:** Without this, Claude Code treats every file in isolation.
...
### 3. Update CLAUDE.md Each Phase

Always update the `## Current Phase` line before starting a new phase.  
This keeps Claude Code focused on what you're building now.

---

## 💬 How to Talk to Claude Code (Phase by Phase)

### Phase 1 — Infrastructure Setup

**Step 1: Ask for explanation first**
```
claude> Read SPEC.md section "Kubernetes Specification" and "Kafka Topic Specification".
...
```
claude> Based on SPEC.md Phase 1 tasks, create a shell script 
...
```
claude> Create k8s/kafka/kafka-cluster.yaml using the Strimzi Kafka CRD 
...
- Ask: *"What happens if I set retention.ms too low on driver.location?"*
...
```
claude> Explain the JWT authentication flow in SPEC.md User Service section.
...
```
claude> Create the user-service in Go at services/user-service/.
...
- If stuck, ask: *"How do I hash a password in Go? Show me just the function, not the full file."*

**Step 4: Generate K8s manifests**
```
claude> Create K8s manifests for user-service in k8s/services/user-service/.
...
```
claude> Draw a state diagram for the order lifecycle from SPEC.md 
...
```
claude> Scaffold order-service in Go at services/order-service/.
...
```
claude> Explain these Kafka producer settings and why each matters 
...
```
claude> In my previous job with a roadside assistance system, we had:
...
```
claude> Explain Redis GEO commands with a real example using driver coordinates 
...
**Step 3: Scaffold**
```
claude> Scaffold matching-service in Go.
...
```
claude> Explain RabbitMQ exchange types with a visual example:
...
```
claude> In SPEC.md, we use Kafka for order events between services,
...
**Step 3: Generate**
```
claude> Create notification-service in Node.js at services/notification-service/.
...
```
claude> Explain the Cache-Aside (Lazy Loading) pattern using the
...
```
claude> In SPEC.md Redis Merchant Service section, we DEL the cache key
...
```
claude> Scaffold merchant-service in Node.js at services/merchant-service/.
...
```javascript
// Fill this in yourself — don't ask Claude Code yet
...
```
claude> Add three Prometheus counters to merchant-service:
...
```
claude> In my getMenu handler, the Redis call is inside a try-catch.
...
```
claude> Compare the on-premise and AWS stacks from SPEC.md side by side.
...
```
claude> Explain IRSA (IAM Roles for Service Accounts) from scratch.
...
```
claude> Update services/notification-service/ to support two modes:
...
|---|---|
| Ask Claude Code to implement everything at once | You won't understand any of it |
| Skip the DRAW step | You lose the mental model |
| Never read generated code | You can't explain it in an interview |
| Use Claude Code to fix every error immediately | Debugging IS learning |
| Skip updating CLAUDE.md between phases | Claude Code loses context, generates inconsistent code |
...
- [ ] Answer: *"What happens if this component goes down?"*
- [ ] Answer: *"How would this behave under 10x load?"*
- [ ] Make a small change to the code without asking Claude Code
...
> *"Walk me through how your system handles a new order from placement to delivery"*
